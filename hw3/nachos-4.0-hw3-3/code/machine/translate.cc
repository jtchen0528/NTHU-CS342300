// translate.cc
//	Routines to translate virtual addresses to physical addresses.
//	Software sets up a table of legal translations.  We look up
//	in the table on every memory reference to find the true physical
//	memory location.
//
// Two types of translation are supported here.
//
//	Linear page table -- the virtual page # is used as an index
//	into the table, to find the physical page #.
//
//	Translation lookaside buffer -- associative lookup in the table
//	to find an entry with the same virtual page #.  If found,
//	this entry is used for the translation.
//	If not, it traps to software with an exception.
//
//	In practice, the TLB is much smaller than the amount of physical
//	memory (16 entries is common on a machine that has 1000's of
//	pages).  Thus, there must also be a backup translation scheme
//	(such as page tables), but the hardware doesn't need to know
//	anything at all about that.
//
//	Note that the contents of the TLB are specific to an address space.
//	If the address space changes, so does the contents of the TLB!
//
// DO NOT CHANGE -- part of the machine emulation
//
// Copyright (c) 1992-1996 The Regents of the University of California.
// All rights reserved.  See copyright.h for copyright notice and limitation
// of liability and disclaimer of warranty provisions.

#include "copyright.h"
#include "main.h"

// Routines for converting Words and Short Words to and from the
// simulated machine's format of little endian.  These end up
// being NOPs when the host machine is also little endian (DEC and Intel).

unsigned int
WordToHost(unsigned int word)
{
#ifdef HOST_IS_BIG_ENDIAN
	register unsigned long result;
	result = (word >> 24) & 0x000000ff;
	result |= (word >> 8) & 0x0000ff00;
	result |= (word << 8) & 0x00ff0000;
	result |= (word << 24) & 0xff000000;
	return result;
#else
	return word;
#endif /* HOST_IS_BIG_ENDIAN */
}

unsigned short
ShortToHost(unsigned short shortword)
{
#ifdef HOST_IS_BIG_ENDIAN
	register unsigned short result;
	result = (shortword << 8) & 0xff00;
	result |= (shortword >> 8) & 0x00ff;
	return result;
#else
	return shortword;
#endif /* HOST_IS_BIG_ENDIAN */
}

unsigned int
WordToMachine(unsigned int word) { return WordToHost(word); }

unsigned short
ShortToMachine(unsigned short shortword) { return ShortToHost(shortword); }

//----------------------------------------------------------------------
// Machine::ReadMem
//      Read "size" (1, 2, or 4) bytes of virtual memory at "addr" into
//	the location pointed to by "value".
//
//   	Returns FALSE if the translation step from virtual to physical memory
//   	failed.
//
//	"addr" -- the virtual address to read from
//	"size" -- the number of bytes to read (1, 2, or 4)
//	"value" -- the place to write the result
//----------------------------------------------------------------------

bool Machine::ReadMem(int addr, int size, int *value)
{
	int data;
	ExceptionType exception;
	int physicalAddress;

	DEBUG(dbgAddr, "Reading VA " << addr << ", size " << size);

	exception = Translate(addr, &physicalAddress, size, FALSE);
	if (exception != NoException)
	{
		RaiseException(exception, addr);
		return FALSE;
	}
	switch (size)
	{
	case 1:
		data = mainMemory[physicalAddress];
		*value = data;
		break;

	case 2:
		data = *(unsigned short *)&mainMemory[physicalAddress];
		*value = ShortToHost(data);
		break;

	case 4:
		data = *(unsigned int *)&mainMemory[physicalAddress];
		*value = WordToHost(data);
		break;

	default:
		ASSERT(FALSE);
	}

	DEBUG(dbgAddr, "\tvalue read = " << *value);
	return (TRUE);
}

//----------------------------------------------------------------------
// Machine::WriteMem
//      Write "size" (1, 2, or 4) bytes of the contents of "value" into
//	virtual memory at location "addr".
//
//   	Returns FALSE if the translation step from virtual to physical memory
//   	failed.
//
//	"addr" -- the virtual address to write to
//	"size" -- the number of bytes to be written (1, 2, or 4)
//	"value" -- the data to be written
//----------------------------------------------------------------------

bool Machine::WriteMem(int addr, int size, int value)
{
	ExceptionType exception;
	int physicalAddress;

	DEBUG(dbgAddr, "Writing VA " << addr << ", size " << size << ", value " << value);

	exception = Translate(addr, &physicalAddress, size, TRUE);
	if (exception != NoException)
	{
		RaiseException(exception, addr);
		return FALSE;
	}
	switch (size)
	{
	case 1:
		mainMemory[physicalAddress] = (unsigned char)(value & 0xff);
		break;

	case 2:
		*(unsigned short *)&mainMemory[physicalAddress] = ShortToMachine((unsigned short)(value & 0xffff));
		break;

	case 4:
		*(unsigned int *)&mainMemory[physicalAddress] = WordToMachine((unsigned int)value);
		break;

	default:
		ASSERT(FALSE);
	}

	return TRUE;
}

//----------------------------------------------------------------------
// Machine::Translate
// 	Translate a virtual address into a physical address, using
//	either a page table or a TLB.  Check for alignment and all sorts
//	of other errors, and if everything is ok, set the use/dirty bits in
//	the translation table entry, and store the translated physical
//	address in "physAddr".  If there was an error, returns the type
//	of the exception.
//
//	"virtAddr" -- the virtual address to translate
//	"physAddr" -- the place to store the physical address
//	"size" -- the amount of memory being read or written
// 	"writing" -- if TRUE, check the "read-only" bit in the TLB
//----------------------------------------------------------------------

ExceptionType
Machine::Translate(int virtAddr, int *physAddr, int size, bool writing)
{
	unsigned i;
	unsigned int vpn, offset;
	TranslationEntry *entry;
	unsigned int pageFrame;

	DEBUG(dbgAddr, "\tTranslate " << virtAddr << (writing ? " , write" : " , read"));

	// check for alignment errors
	if (((size == 4) && (virtAddr & 0x3)) || ((size == 2) && (virtAddr & 0x1)))
	{
		DEBUG(dbgAddr, "Alignment problem at " << virtAddr << ", size " << size);
		return AddressErrorException;
	}

	// we must have either a TLB or a page table, but not both
	ASSERT(tlb == NULL || pageTable == NULL);
	ASSERT(tlb != NULL || pageTable != NULL);

	// calculate the virtual page number, and offset within the page,
	// from the virtual address
	vpn = (unsigned)virtAddr / PageSize;
	offset = (unsigned)virtAddr % PageSize;
	entry = NULL;
	if (tlb == NULL) // => page table => Inverted Page Table
	{
		//直接調用SearchPageTable函數來查找
		entry = SearchPageTable(vpn, &pageFrame);

		//如果出現pageFault的話，則進行pageReplacement
		if (entry == NULL)
		{
			kernel->stats->numPageFaults++;
			entry = PageReplacement(SecondChance, vpn, &pageFrame);
		}
		//end
	}
	else
	{
		for (entry = NULL, i = 0; i < TLBSize; i++)
		{
			if (tlb[i].valid && (tlb[i].virtualPage == vpn))
			{
				entry = &tlb[i]; // FOUND!
				break;
			}
		}
		if (entry == NULL)
		{ // not found
			DEBUG(dbgAddr, "Invalid TLB entry for this virtual page!");
			return PageFaultException; // really, this is a TLB fault,
			// the page may be in memory,
			// but not in the TLB
		}
	}

	if (entry->readOnly && writing)
	{ // trying to write to a read-only page
		DEBUG(dbgAddr, "Write to read-only page at " << virtAddr);
		return ReadOnlyException;
	}
	//pageFrame = entry->physicalPage;

	// if the pageFrame is too big, there is something really wrong!
	// An invalid translation was loaded into the page table or TLB.
	if (pageFrame >= NumPhysPages)
	{
		DEBUG(dbgAddr, "Illegal pageframe " << pageFrame);
		return BusErrorException;
	}
	entry->use = TRUE; // set the use, dirty bits
	if (writing)
		entry->dirty = TRUE;
	*physAddr = pageFrame * PageSize + offset;
	ASSERT((*physAddr >= 0) && ((*physAddr + size) <= MemorySize));
	DEBUG(dbgAddr, "phys addr = " << *physAddr);
	return NoException;
}

/*
* Machine::SearchPageTable 搜索pageTable，查看是否有vpn頁。
* 如果有則返該頁對應的實體分頁，否則返回null
*/
TranslationEntry *
Machine::SearchPageTable(unsigned int vpn, unsigned *pageframe)
{
	for (unsigned i = 0; i < NumPhysPages; i++)
	{
		if (!pageTable[i].valid)
		{
			kernel->stats->numPageFaults++;

			//將虛擬分頁的資料從硬碟Swap In至實體分頁
			SwapIn(vpn, i);

			//實體分頁＝ 分頁表的索引i
			*pageframe = i;

			pageTable[i].pid = kernel->currentThread->getPid();
			pageTable[i].virtualPage = vpn;
			pageTable[i].valid = TRUE;

			//將分頁索引 i 紀錄到FIFO的queue裡
			FIFOqueue->Append(i);

			//將分頁索引 i 紀錄到LRU的Stack裡
			LRUstack->Append(i);

			return &pageTable[i];
		}
		else if (pageTable[i].valid &&
				 (pageTable[i].pid == kernel->currentThread->getPid()) &&
				 (pageTable[i].virtualPage == vpn))
		{
			//將經常被使用的分頁索引i 移到LRU Stack的最上面（本實做是將queue倒過來當成stack用）
			LRUstack->Remove(i);
			LRUstack->Append(i);

			//實體分頁 ＝ 分頁表的索引 i
			*pageframe = i;

			return &pageTable[i];
		}
	}
	return NULL;
}

/*
* Machine::PageReplacement 當實體分頁都被佔滿時,要執行pageReplacement，有四種不同的方法：FIFO, Random, SecondChance, LRU
* mode為採用的模式, vpn為要被執行的虛擬頁，pageframe為要返回的實體頁地址
*/
/*
FIFO:
1.建立一個FIFO的queue。
2.當虛擬分頁載入記憶體時，將其分頁加入queue尾端。
3.當PageReplacement發生時，移除Queue的第一個內容當成victim，並將victim加到queue的尾端。
 
Random:
1.當PageReplacement發生時，直接取隨機變數除與32的餘數當作victim。
2.公式：victim=rand()%32
 
SecondChance:
1.使用FIFO法，搭配Reference Bit 。
2.當PageReplacement發生時，檢查Reference Bit，若為true時，不選為victim，且將Reference Bit設為false，直到檢查出
 
Reference Bit為false時，就選為victim。
 
LRU:
1.建立一個LRU的Stack
2.當虛擬分頁載入記憶體時，將其分頁push Stack中。
3.當實體分頁被讀取時，將其分頁從Stack中移除再push至Stack中。
3.當PageReplacement發生時，移除Stack中最下面的內容當成victim，並將victim push 至Stack中。
*/
TranslationEntry *
Machine::PageReplacement(PageReplacementMode mode, unsigned int vpn, unsigned *pageframe)
{
	unsigned int victim; //要被替換的分頁
	switch (mode)
	{
	//如果是FIFO的話，則把queue中最前面的元素先移除，再在queue的末尾插入新元素
	case FIFO:
		victim = FIFOqueue->RemoveFront();
		FIFOqueue->Append(victim);
		Break;
	//取隨機變數除與32的餘數當作victim page
	case Random:
		victim = rand() % 32;
		break;
		//給它第二次的機會
	case SecondChance:
		victim = FIFOqueue->RemoveFront();
		FIFOqueue->Append(victim);
		while (pageTable[victim].use == TRUE)
		{
			pageTable[victim].use = FALSE;
			//再給它一次機會
			victim = FIFOqueue->RemoveFront();

			FIFOqueue->Append(victim);
		}
		break;
	//移除stack最上面的元素，並且append新元素
	case LRU:
		victim = LRUstack->RemoveFront();
		LRUstack->Append(victim);
	}

	//將victim page內容寫回硬碟中
	SwapOut(victim, pageTable[victim].virtualPage);

	//將要執行的虛擬分頁載入victim page 中
	SwapIn(vpn, victim);

	pageTable[victim].pid = kernel->currentThread->getPid();
	pageTable[victim].virtualPage = vpn;
	*pageframe = victim;
	return &pageTable[victim];
}

/*
* Machine::SwapIn 將虛擬分頁從硬碟載入實體分頁中。
* virtPage為虛擬分頁，physPage為實體分頁
*/
void Machine::SwapIn(unsigned int virtPage, unsigned int physPage)
{
	unsigned int sectorNum;
	char *buf;
	buf = new char[PageSize];

	//讀取在虛擬分頁在硬碟的Sector Number
	sectorNum = kernel->currentThread->space->virtMemorySpace[virtPage];

	//讀取硬碟資料到buf裡
	kernel->virtualMemory->ReadSector(sectorNum, buf);

	//將buf資料寫入主記憶體裡
	bcopy(buf, &mainMemory[physPage * PageSize], PageSize);
}

/*
* Machine::SwapOut 將實體分頁從主記憶體存回硬碟中。
* virtPage為虛擬分頁，physPage為實體分頁
*/
void Machine::SwapOut(unsigned int physPage, unsigned int virtPage)
{
	unsigned int sectorNum;
	char *buf;
	buf = new char[PageSize];

	//只存回屬於currentThread pid的分頁
	if (kernel->currentThread->getPid() == pageTable[physPage].pid)
	{
		buf = new char[PageSize];

		//讀取主記憶體資料到buf裡
		bcopy(&mainMemory[physPage * PageSize], buf, PageSize);

		//讀取在虛擬分頁在硬碟的Sector Number
		sectorNum = kernel->currentThread->space->virtMemorySpace[virtPage];

		//將buf寫入到硬碟中
		kernel->virtualMemory->WriteSector(sectorNum, buf);
	}
}